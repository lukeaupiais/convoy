...
Tickets may have typed, directed relations to other tickets in the same project.
The relation kind is supplied by the caller; Work does not interpret it. Existing
support-to-development links are retained for published workflow compatibility.
New ticket creation can use a board's configured work type and first status column.
...
binding membership. Each board can create its own work type. Relations are
durable and independent of board placement or status.
